...
$icons = [
    ...
    'upcoming-deadlines'=> 'M8 7V3m8 4V3m-9 8h10m-11 8h12a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z',
...
$colorMap = [
    ...
    'pink'   => ['bg' => 'bg-pink-500',    'light' => 'bg-pink-400',   'hover' => 'group-hover:text-pink-500'],
...
<a href="{{ $href }}"
class="block shrink-0 group relative w-[250px] h-[150px] px-6 py-4 rounded-2xl shadow-md
        bg-white/30 dark:bg-[#1e1f1e] backdrop-blur-sm
        hover:bg-[#efefef] dark:hover:bg-[#272927] hover:shadow-xl hover:-translate-y-1
        transition-all duration-300 cursor-pointer">

{{-- Status badge --}}
<div class="absolute top-0 left-0 rounded-tl-2xl rounded-br-2xl h-9 w-[120px]
            group-hover:w-[130px] transition-all duration-300
            shadow-md flex items-center justify-center gap-1 overflow-hidden {{ $c['bg'] }}">
    <svg class="w-3 h-3 text-white shrink-0" fill="currentColor" viewBox="0 0 20 20">
        <path fill-rule="evenodd"
            d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
            clip-rule="evenodd" />
    </svg>
    <span class="text-white text-xs font-semibold truncate">{{ $title }}</span>
</div>
...
<div class="flex flex-col justify-between h-full pt-9">
    ...
    <div class="flex items-center gap-2">
        <div class="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 {{ $c['light'] }}">
            <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="{{ $icons[$type] ?? $icons['tasks'] }}" />
            </svg>
        </div>
        <span class="text-xs text-gray-500 dark:text-[#c5c5c5] font-medium leading-tight">{{ $description }}</span>
    </div>

    {{-- Number --}}
    <p class="text-4xl font-bold text-gray-800 dark:text-white transition-colors duration-300 {{ $c['hover'] }}">
        {{ $number }}
    </p>
</div>
